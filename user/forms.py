...
class RegForm(forms.Form):
    ...
    password = forms.CharField(label='密码',
                               ...
                               widget=forms.PasswordInput(
                                   attrs={'class':'form-control', 'placeholder':'请输入密码'}))
    password_again = forms.CharField(label='再填一次密码',
                                     min_length=6,
                                     widget=forms.PasswordInput(
    ...
    def clean(self):
        # 验证输入的验证码是否正确
        code = self.request.session.get('register_code', '')    # 从session获取注册用的code
...
class BindEmailForm(forms.Form):
    ...
    def clean(self):
        ...
        if self.request.user.email != '':
            raise forms.ValidationError('你已被绑定邮箱')

        # 验证输入的验证码是否正确
        code = self.request.session.get('bind_email_code', '')  # 从session获取修改邮箱用的code
        verification_code = self.cleaned_data.get('verification_code', '')
        if not (code != '' and code == verification_code):
            raise forms.ValidationError('验证码不正确')
...
class ChangePasswordForm(forms.Form):
    ...
    new_password_again = forms.CharField(label='再次输入新密码',
                                         widget=forms.PasswordInput(
                                            attrs={'class':'form-control', 'placeholder':'请输入再次输入密码'}))
...
class ForgetPasswordForm(forms.Form):
    ...
    def clean_email(self):
        ...
        if not User.objects.filter(email=email).exists():
            raise forms.ValidationError('此邮箱未绑定用户')
        return email
    ...
    def clean_verification_code(self):
        ...
        # 验证输入的验证码是否正确
        code = self.request.session.get('forget_password_code', '')    # 从session获取注册用的code
        verification_code = self.cleaned_data.get('verification_code', '')
        if not (code != '' and code == verification_code):
            raise forms.ValidationError('验证码不正确')
