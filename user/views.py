...
def login(request):
    if request.method == 'POST':
        login_form = LoginForm(request.POST)  # 提交登录数据
        if login_form.is_valid():
            user = login_form.cleaned_data['user']  # 获取已验证通过数据用于登录
            auth.login(request, user)
            return redirect(request.GET.get('from', reverse('home')))  # 跳转回原页面
    else:
        login_form = LoginForm()  # 获取登录页面
    context = {}
    context['login_form'] = login_form
    return render(request, 'user/login.html', context)  # 渲染登录页面
...
def send_verification_code(request):
    ...
    if email != '':
        ...
        if now - send_code_time < 30:
            ...
            # 发送邮件
            send_mail('绑定邮箱', '验证码：%s' % code, 'yf2913128@outlook.com', [email], fail_silently=False, )
            data['status'] = 'SUCCESS'
    else:
        data['status'] = 'ERROR'
    return JsonResponse(data)
